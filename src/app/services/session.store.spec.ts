...
describe('sessionStore', () => {
  ...
  describe('placeScore', () => {
    test('should place score in ONE column on first placement', () => {
      ...
      expect(readScoreCell(store.games()[0], GAME_COLUMN.one, SCORE_CATEGORY.aces)).toEqual({
        value: 5,
        isScratched: false,
      });
    });
    ...
    test('should advance to TWO column after ONE is filled', () => {
      ...
      expect(readScoreCell(store.games()[0], GAME_COLUMN.two, SCORE_CATEGORY.aces)).toBeDefined();
    });
    ...
    test('should mark scratch when score is 0', () => {
      ...
      const cell = readScoreCell(store.games()[0], GAME_COLUMN.one, SCORE_CATEGORY.aces);
    ...
    test('should place independently in game 1', () => {
      ...
      expect(readScoreCell(store.games()[1], GAME_COLUMN.one, SCORE_CATEGORY.aces)).toBeDefined();
      expect(readScoreCell(store.games()[0], GAME_COLUMN.one, SCORE_CATEGORY.aces)).toBeUndefined();
    });
  ...
  describe('undo', () => {
    test('should restore previous game state', () => {
      store.setCurrentDice([5, 0, 0, 0, 0, 0] as DiceSet);
      store.placeScore(SCORE_CATEGORY.aces, 0);
      expect(readScoreCell(store.games()[0], GAME_COLUMN.one, SCORE_CATEGORY.aces)).toBeDefined();
